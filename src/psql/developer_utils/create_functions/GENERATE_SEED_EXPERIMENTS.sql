...
BEGIN
	...
	SELECT
		EXPERIMENTS_TO_CREATE = COUNT(*) - GC.STARTING_POPULATION_SIZE
	FROM
		EXPERIMENT E
	JOIN
		EXPERIMENT_GROUP EG
	...
	IF EXPERIMENTS_TO_CREATE > 0 THEN
		...
		SELECT FILL_IN_MISSING_PARAMETERS(NEWLY_CREATED_EXPERIMENT_ID);
		
		SELECT GENERATE_SEED_EXPERIMENTS(PARAM_EXPERIMENT_GROUP_ID); -- SAFE, BUT I/O HEAVY WAY TO GUARANTEE WE DON'T OVERFILL THE STARTING POPULATION FOR THIS EXPERIMENT_GROUP
	END IF;
END
$$
LANGUAGE PLPGSQL;	